...
void loop() {
  ...
  for(int ct_av=0;ct_av<averages;ct_av++) {
  ...
  sensorValue=sensor_linearisieren(sensorValue);
...
int sensor_linearisieren(int sensorValue) 
{
  sensorValue=abs(sensorValue-25);
  float sens_op=(float)sensorValue/200;
  sens_op=sqrt(sens_op);
  sens_op=sens_op*200;
  sensorValue=(int)sens_op;
  return sensorValue;
}
