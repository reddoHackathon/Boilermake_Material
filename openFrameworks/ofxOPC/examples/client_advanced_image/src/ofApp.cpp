...
void ofApp::setup()
{
    ...
    //allocate texture
    screengrab.allocate(grid.getWidth(),grid.getHeight(),GL_RGB);
    
    //init heart template and set image
    heart.setup();
...
void ofApp::draw()
{
    ...
    int px = 300;
    int py = 200;
    //draw brush on mouse position
    brush.draw(mouseX - brush.getWidth()/2, mouseY - brush.getHeight()/2);
    //capture image
    screengrab.loadScreenData(px,py,screengrab.getWidth(),screengrab.getHeight());
    //load image to pixels and mirror
