...
void ofxOPC::writeChannel(uint8_t channel, vector<ofColor>pix)
{
    ...
    for (int i = 0; i < pix.size(); i++)
    {
        ...
        //cout << (int)pix[i].r << " " << (int)pix[i].g << " " << (int)pix[i].b << " ";
    }
...
void ofxOPC::writeChannels(vector<ofColor>pix)
{
    // Bail early if there's no pixel data
    if(pix.empty())
    {
...
int ofxOPC::getPacketLength(){
    return OPC_SPC_packet_length;
}
...
void ofxOPC::sendRawBytes(const char* msg, const int numBytes){
    client.sendRawBytes(msg, numBytes);
}

void ofxOPC::sendRawMessage(vector<ofColor>pix){
    // Bail early if there's no pixel data
    if(pix.empty())
    {
        return;
    ...
    // Send the data
    client.sendRawMsg((char *)(OPC_SPC_packet), OPC_SPC_packet_length);
}
